    ...
    for i in range(1,5):
        ...
        if i == 1:
            try:
                ...
                c.execute(f"CREATE TABLE {name} (date_nr real, date text, value1 integer, value2 integer, value3 integer, value4 integer)")
            except:
                print("Exception 1")
        if i == 2:
            try:
        ...
        if i == 3:
            try:
                print(f"{name} created")
                c.execute(f"CREATE TABLE {name} (date_nr real, date text, value1 integer, value2 integer)")
            except:
                print("Exception 3")
        ...
        if i == 4:
            try:
                print(f"{name} created")
                c.execute(f"CREATE TABLE {name} (date_nr real, date text, value1 integer, value2 integer, value3 integer)")
            except:
                print("Exception 4")
        ...
        for number, line in enumerate(lines):
            ...
            if i == 1:
                print("1 ausgeführt")
                adding = (fields[0], f"{mytime.year}-{str(mytime.month).zfill(2)}-{str(mytime.day).zfill(2)} {str(mytime.hour).zfill(2)}:{str(mytime.minute).zfill(2)}:{str(mytime.second).zfill(2)}",
